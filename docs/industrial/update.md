...
![Firmware Version](assets/industrial_ha_device_info_card.png){width="250"}
...
1. Find the latest firmware [here](https://github.com/nplan/HomeButtons/releases){:target="_blank"}. Check release notes for any special update requirements. Make sure to select the correct file ending with `_industrial.bin`. Download it to your computer.

2. Enter setup by going to the [*Settings Menu*](#settings) and pressing the :red_circle: red button. 
...
1. Connect *Home Buttons* to your computer using the USB-C cable.
...
2. Find the latest firmware [here](https://github.com/nplan/HomeButtons/releases){:target="_blank"}. Check release notes for any special update requirements. Make sure to select the correct file:
    ...
    1. **Update image** - regular update
        ...
        Select file ending with `_industrial.bin`.

    2. **Full image** - to flash the entire device memory. This will erase all user data.

        Select file ending with `_industrial_full_image.bin`. 
...
`BOOT LED` will light up. Release both buttons.
    ...
    ![programming Mode](assets/hb_ind_open.png){width="350"}

5. Connect *Home Buttons* to your computer using the USB-C cable.
