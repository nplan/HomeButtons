...
// ------ function prototypes ------
bool connect_wifi(bool save_channel_bssid = false);
...
void setup() {
  ...
  if (low_batt_mode) {
    ...
    if (batt_volt < MIN_BATT_VOLT) {
      eink::display_please_recharge_screen();
      low_batt_mode = true;
      save_preferences();
      start_esp_sleep();
    }
  ...
  if (!wifi_done) {
    ...
    switch (boot_reason) {
      ...
      case TMR: control_flow = TIMER; break;
  ...
  switch (control_flow) {
    case WIFI_SETUP: {
      ...
      if (!wifi_connected && !connect_wifi()) { // double check that wifi can not be connected with connect_wifi() function
        wifi_done = false;
        save_preferences();
        eink::display_welcome_screen(unique_id.c_str());
        break;
      }

      connect_wifi(true); // connect to save channel and bssid
    ...
    case SETUP: {
      ...
      if (!mqtt_connected) {
        ...
        delay(3000);
        eink::display_setup_required_screen();
        break;
      }
      else {
        set_topics();
...
bool connect_wifi(bool save_channel_bssid) {
  WiFi.mode(WIFI_STA);
  WiFi.persistent(true);

  // Start by trying to connect with settings stored by ESP32, channel and BSSID fixed
  WiFi.begin();
  wifi_start_time = millis();
  while (true) {
      delay(50);
      if (WiFi.status() == WL_CONNECTED) {
        if (save_channel_bssid) {
          break;
        }
        else {
          return true;
...
void read_preferences() {
  ...
  unique_id = preferences.getString("unique_id", 
              String("HBTNS-") + serial_number + "-" + RANDOM_ID);
  preferences.end();

  preferences.begin("user", true);
  initialized = preferences.getBool("initialized", false);
  low_batt_mode = preferences.getBool("low_batt_mode", false);
...
void save_preferences() {
  ...
  preferences.putBool("low_batt_mode", low_batt_mode);
