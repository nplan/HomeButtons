...
class State {
public:
    explicit State(StateMachineDefinition &stateMachineDefinition) : m_stateMachine(stateMachineDefinition) {}
...
public:
    ...
    StateMachine(const char *name, StateMachineType &stateMachineType) : m_currentState(&std::get<0>(m_states)),
    ...
    template<typename NextState>
    void transitionTo() {
        log_i("Leaving state %s::%s", m_name, std::visit([](auto statePtr) { return statePtr->getName(); }, m_currentState));
        std::visit([](auto statePtr) { statePtr->exit(); }, m_currentState);

        m_currentState = &std::get<NextState>(m_states);

        log_i("Entering state %s::%s", m_name, std::visit([](auto statePtr) { return statePtr->getName(); }, m_currentState));
