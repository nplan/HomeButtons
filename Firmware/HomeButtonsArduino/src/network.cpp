  ...
  while (true) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        while (true) {
          delay(50);
          if (WiFi.status() == WL_CONNECTED) {
            persisted_s.wifi_quick_connect = true; // can connect next time with params saved by esp
            return true;
          }
          else if (millis() - wifi_start_time > WIFI_TIMEOUT) {
            persisted_s.wifi_quick_connect = false; // make sure next time quick connect is disabled
            return false;
          }
        }
      }
...
bool connect_mqtt() {
  ...
  while(!client.connected()) {
    if (user_s.mqtt_user.length() > 0 && user_s.mqtt_password.length() > 0) {
      client.connect(factory_s.unique_id.c_str(), user_s.mqtt_user.c_str(), user_s.mqtt_password.c_str());
    }
    else {
